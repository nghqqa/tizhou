...
// ---- 导入质量分级（逐题 + 套件汇总）----
// 不代表 OCR 准确率，仅反映结构完整度和原图保留情况。
// structured = 题干+选项结构完整且无未隔离噪声；
// image-backed = 题干关联原图但选项不完整（图推等）；
// unsupported = 无法可靠切题。failed 由转换异常时标记。

export type QuestionQuality = 'structured' | 'image-backed' | 'unsupported'
...
export interface ImportQualitySummary {
  ...
  unsupported: number
  /** 自动生成的待审核题目数 */
  autoGeneratedQuestions: number
  /** 嫌疑丢题数（套号连续性断裂或题号跳跃），非确认丢题 */
  suspectedLossCount: number
  /** 原图保留率 = 有关联图片的页数 / 总输入页数 */
  originalImageRetentionRate: number
}

export function computeImportSummary(input: {
  totalPages: number
  items: Array<{
    stem: string
    optionCount: number
  ...
  for (const item of input.items) {
    const complete = item.stem.length >= 8 && item.optionCount >= 2
    if (item.hasImage && !complete) imageBacked += 1
    else if (complete) structured += 1
    else unsupported += 1
  }
  const totalQuestions = input.items.length
  ...
  return {
    ...
    imageBacked,
    unsupported,
    autoGeneratedQuestions: totalQuestions,
    suspectedLossCount: Math.max(0, totalPages * 5 - totalQuestions),
    originalImageRetentionRate: Math.round((imageBacked / totalPages) * 100) / 100
  }
}
